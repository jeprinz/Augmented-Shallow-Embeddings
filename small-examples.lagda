...
consistency : ∀{t} → Term {S.∅} ∅ (S.Π₁ S.U₀ (S.var (S.same))) t → ⊥
consistency e = ((extract e) tt) ⊥
\end{code}
...
idExample : Term ∅ (S.Π₁ S.U₀ (S.cumu (S.Π₀ (S.var S.same) (S.var (S.next S.same))))) _
idExample = lambda (lambda (var same))

idUExample : Term ∅ (S.Π₁ S.U₀ S.U₀) _
idUExample = lambda (var same)

example : Term ∅ (λ _ → (X Y : Set) → (X → Y) → X → Y) _
example = lambda (lambda (lambda (lambda (app (var (next same)) (var same)))))

-- Leibniz equality
-- leibniz : ∀{Γ} → (T : S.Type Γ) → (a b : S.Term Γ T) → S.Type Γ
-- leibniz T a b = λ γ → (P : (T γ) →
leibniz : (T : Set) → T → T → Set₁
leibniz T a b = (P : T → Set) → P a → P b

-- trans : Term ∅ (λ _ → (T : Set) → (a b c : T)
